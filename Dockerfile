###########################################
##  Google App Engine go1.12 stable
###########################################
FROM ubuntu:18.04
MAINTAINER @eaglesakura

ARG GCLOUD_INSTALL_VERSION="278.0.0"
...
RUN  apt update \
  && mkdir $HOME/tools \
  && apt install -y jq python git wget curl unzip tzdata build-essential openjdk-8-jre-headless

###########################################
## Install gcloud tools
###########################################
ENV FIRESTORE_EMULATOR_HOST="localhost:9001"
...
RUN  cd $HOME/tools  \
  ...
  && yes | gcloud components install beta app-engine-go cloud-firestore-emulator
